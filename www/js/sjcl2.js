...
sjcl.cipher.aes.prototype = {encrypt: function(b) {
        ...
        var R = this.k[0], Q = this.k[1], P = R[4], O = Q[4], N, z, y, v = [], r = [], t, j, o, i;
        for (N = 0; 256 > N; N++) {
            r[(v[N] = N << 1 ^ 283 * (N >> 7)) ^ N] = N
        }
        for (z = y = 0; !P[z]; z ^= t || 1, y = r[y] || 1) {
            o = y ^ y << 1 ^ y << 2 ^ y << 3 ^ y << 4;
            o = o >> 8 ^ o & 255 ^ 99;
            P[z] = o;
            O[o] = z;
            j = v[N = v[t = v[z]]];
            i = 16843009 * j ^ 65537 * N ^ 257 * t ^ 16843008 * z;
            j = 257 * v[o] ^ 16843008 * o;
            for (N = 0; 4 > N; N++) {
                R[N][z] = j = j << 24 ^ j >>> 8, Q[N][o] = i = i << 24 ^ i >>> 8
            }
        }
        for (N = 0; 5 > N; N++) {
...
sjcl.bitArray = {bitSlice: function(e, d, f) {
        ...
        return f === s ? e : sjcl.bitArray.clamp(e, f - d)
...
sjcl.codec.base64 = {J: "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/", fromBits: function(j, i, p) {
...
sjcl.hash.sha256.prototype = {blockSize: 512, reset: function() {
        ...
        var d, h = this.o = sjcl.bitArray.concat(this.o, f);
        d = this.h;
        f = this.h = d + sjcl.bitArray.bitLength(f);
        try {
            for (d = 512 + d & -512; d <= f; d += 512) {
                x(this, h.splice(0, 16))
            }
        } catch (g) {
        }
        return this
    }, finalize: function() {
        var f, d = this.o, h = this.r, d = sjcl.bitArray.concat(d, [sjcl.bitArray.partial(1, 1)]);
        for (f = d.length + 2; f & 15; f++) {
            d.push(0)
        }
        d.push(Math.floor(this.h / 4294967296));
        try {
            for (d.push(this.h | 0); d.length; ) {
                x(this, d.splice(0, 16))
            }
        } catch (g) {
        }
        this.reset();
...
function x(V, U) {
    var T, S, R, Q = U.slice(0), P = V.r, O = V.b, z = P[0], N = P[1], o = P[2], y = P[3], j = P[4], X = P[5], i = P[6], W = P[7];
    for (T = 0; 64 > T; T++) {
        16 > T ? S = Q[T] : (S = Q[T + 1 & 15], R = Q[T + 14 & 15], S = Q[T & 15] = (S >>> 7 ^ S >>> 18 ^ S >>> 3 ^ S << 25 ^ S << 14) + (R >>> 17 ^ R >>> 19 ^ R >>> 10 ^ R << 15 ^ R << 13) + Q[T & 15] + Q[T + 9 & 15] | 0), S = S + W + (j >>> 6 ^ j >>> 11 ^ j >>> 25 ^ j << 26 ^ j << 21 ^ j << 7) + (i ^ j & (X ^ i)) + O[T], W = i, i = X, X = j, j = y + S | 0, y = o, o = N, N = z, z = S + (N & o ^ y & (N ^ o)) + (N >>> 2 ^ N >>> 13 ^ N >>> 22 ^ N << 30 ^ N << 19 ^ N << 10) | 0
    }
    P[0] = P[0] + z | 0;
    P[1] = P[1] + N | 0;
    P[2] = P[2] + o | 0;
...
sjcl.mode.ccm = {name: "ccm", encrypt: function(y, v, t, r, p) {
        var o, n = v.slice(0), m = sjcl.bitArray, i = m.bitLength(t) / 8, j = m.bitLength(n) / 8;
        p = p || 64;
        r = r || [];
        ...
        o < 15 - i && (o = 15 - i);
        t = m.clamp(t, 8 * (15 - o));
        v = sjcl.mode.ccm.L(y, v, t, r, p, o);
        n = sjcl.mode.ccm.p(y, n, t, v, p, o);
        return m.concat(n.data, n.tag)
    }, decrypt: function(y, v, t, r, p) {
        p = p || 64;
        r = r || [];
        var o = sjcl.bitArray, n = o.bitLength(t) / 8, m = o.bitLength(v), i = o.clamp(v, m - p), j = o.bitSlice(v, m - p), m = (m - p) / 8;
        7 > n && q(new sjcl.exception.invalid("ccm: iv must be at least 7 bytes"));
        for (v = 2; 4 > v && m >>> 8 * v; v++) {
        }
        v < 15 - n && (v = 15 - n);
        t = o.clamp(t, 8 * (15 - v));
        i = sjcl.mode.ccm.p(y, i, t, j, p, v);
        y = sjcl.mode.ccm.L(y, i.data, t, r, p, v);
        o.equal(i.tag, y) || q(new sjcl.exception.corrupt("ccm: tag doesn't match"));
        return i.data
    }, L: function(t, r, p, o, n, m) {
        ...
        if (o.length) {
            ...
            for (o = 0; o < k.length; o += 4) {
                m = t.encrypt(i(m, k.slice(o, o + 4).concat([0, 0, 0])))
            }
        }
        for (o = 0; o < r.length; o += 4) {
            m = t.encrypt(i(m, r.slice(o, o + 4).concat([0, 0, 0])))
        }
        return j.clamp(m, 8 * n)
    }, p: function(y, v, t, r, p, o) {
        var n, m = sjcl.bitArray;
        n = m.l;
        var i = v.length, j = m.bitLength(v);
        t = m.concat([m.partial(8, o - 1)], t).concat([0, 0, 0]).slice(0, 4);
        r = m.bitSlice(n(r, y.encrypt(t)), 0, p);
        if (!i) {
            return{tag: r, data: []}
        }
        for (n = 0; n < i; n += 4) {
            t[3]++, p = y.encrypt(t), v[n] ^= p[0], v[n + 1] ^= p[1], v[n + 2] ^= p[2], v[n + 3] ^= p[3]
        }
        return{tag: r, data: m.clamp(v, j)}
    }};
sjcl.mode.ocb2 = {name: "ocb2", encrypt: function(R, Q, P, O, N, z) {
        128 !== sjcl.bitArray.bitLength(P) && q(new sjcl.exception.invalid("ocb iv must be 128 bits"));
        var y, v = sjcl.mode.ocb2.H, r = sjcl.bitArray, t = r.l, j = [0, 0, 0, 0];
        P = v(R.encrypt(P));
        var o, i = [];
        O = O || [];
        N = N || 64;
        for (y = 0; y + 4 < Q.length; y += 4) {
            o = Q.slice(y, y + 4), j = t(j, o), i = i.concat(t(P, R.encrypt(t(P, o)))), P = v(P)
        }
        o = Q.slice(y);
        Q = r.bitLength(o);
        y = R.encrypt(t(P, [0, 0, 0, Q]));
        o = r.clamp(t(o.concat([0, 0, 0]), y), Q);
        j = t(j, t(o.concat([0, 0, 0]), y));
        j = R.encrypt(t(j, t(P, v(P))));
        O.length && (j = t(j, z ? O : sjcl.mode.ocb2.pmac(R, O)));
        ...
        Q = Q || 64;
        var O = sjcl.mode.ocb2.H, N = sjcl.bitArray, y = N.l, z = [0, 0, 0, 0], o = O(U.encrypt(S)), v, j, V = sjcl.bitArray.bitLength(T) - Q, i = [];
        R = R || [];
        for (S = 0; S + 4 < V / 32; S += 4) {
            v = y(o, U.decrypt(y(o, T.slice(S, S + 4)))), z = y(z, v), i = i.concat(v), o = O(o)
        }
        j = V - 32 * S;
        v = U.encrypt(y(o, [0, 0, 0, j]));
        v = y(v, N.clamp(T.slice(S), j).concat([0, 0, 0]));
        z = y(z, v);
        z = U.encrypt(y(z, y(o, O(o))));
        R.length && (z = y(z, P ? R : sjcl.mode.ocb2.pmac(U, R)));
        N.equal(N.clamp(z, Q), N.bitSlice(T, V)) || q(new sjcl.exception.corrupt("ocb: tag doesn't match"));
...
sjcl.mode.gcm = {name: "gcm", encrypt: function(h, g, l, k, j) {
        var i = g.slice(0);
        g = sjcl.bitArray;
        k = k || [];
        ...
        var O, N, y, z, o, v, j, V, i = sjcl.bitArray;
        v = S.length;
        j = i.bitLength(S);
        V = i.bitLength(R);
        N = i.bitLength(Q);
        ...
        for (z = 0; z < v; z += 4) {
            o[3]++, y = T.encrypt(o), S[z] ^= y[0], S[z + 1] ^= y[1], S[z + 2] ^= y[2], S[z + 3] ^= y[3]
        }
        S = i.clamp(S, j);
        U && (R = sjcl.mode.gcm.g(O, N, S));
...
sjcl.misc.pbkdf2 = function(N, z, y, v, t) {
    y = y || 1000;
    (0 > v || 0 > y) && q(sjcl.exception.invalid("invalid params to pbkdf2"));
    "string" === typeof N && (N = sjcl.codec.utf8String.toBits(N));
    "string" === typeof z && (z = sjcl.codec.utf8String.toBits(z));
    t = t || sjcl.misc.hmac;
    N = new t(N);
    var r, p, o, j, m = [], i = sjcl.bitArray;
    for (j = 1; 32 * m.length < (v || 1); j++) {
        t = r = N.encrypt(i.concat(z, [j]));
        for (p = 1; p < y; p++) {
            r = N.encrypt(r);
...
sjcl.prng.prototype = {randomWords: function(i, h) {
        var n = [], m;
        m = this.isReady(h);
        var l;
        ...
        switch (typeof t) {
            ...
            case"object":
                ...
                if ("[object Uint32Array]" === p) {
                    ...
                    "[object Array]" !== p && (i = 1);
                    for (p = 0; p < t.length && !i; p++) {
                        "number" !== typeof t[p] && (i = 1)
                    }
                }
                if (!i) {
                    if (r === s) {
                        for (p = r = 0; p < t.length; p++) {
                            for (n = t[p]; 0 < n; ) {
                                r++, n >>>= 1
                            }
                        }
                    }
                    this.c[k].update([o, this.C++, 2, r, m, t.length].concat(t))
                }
                break;
            case"string":
                r === s && (r = t.length);
                this.c[k].update([o, this.C++, 3, r, m, t.length]);
                this.c[k].update(t);
                break;
            default:
                i = 1
        ...
        this.q || (this.a = {loadTimeCollector: D(this, this.aa), mouseCollector: D(this, this.ba), keyboardCollector: D(this, this.$), accelerometerCollector: D(this, this.U), touchCollector: D(this, this.da)}, window.addEventListener ? (window.addEventListener("load", this.a.loadTimeCollector, u), window.addEventListener("mousemove", this.a.mouseCollector, u), window.addEventListener("keypress", this.a.keyboardCollector, u), window.addEventListener("devicemotion", this.a.accelerometerCollector, u), window.addEventListener("touchmove", this.a.touchCollector, u)) : document.attachEvent ? (document.attachEvent("onload", this.a.loadTimeCollector), document.attachEvent("onmousemove", this.a.mouseCollector), document.attachEvent("keypress", this.a.keyboardCollector)) : q(new sjcl.exception.bug("can't attach event")), this.q = !0)
        ...
        b = b.touches[0] || b.changedTouches[0];
        sjcl.random.addEntropy([b.pageX || b.clientX, b.pageY || b.clientY], 1, "touch");
        E(0)
    }, aa: function() {
        E(2)
    }, U: function(d) {
...
sjcl.json = {defaults: {v: 1, iter: 1000, ks: 128, ts: 64, mode: "ccm", adata: "", cipher: "aes"}, Y: function(i, h, n, m) {
        n = n || {};
        m = m || {};
        ...
        "string" === typeof n && (k.adata = n = sjcl.codec.utf8String.toBits(n));
        ...
        var e = {}, h, g;
        for (h = 0; h < f.length; h++) {
            (g = f[h].match(/^\s*(?:(["']?)([a-z][a-z0-9]*)\1)\s*:\s*(?:(-?\d+)|"([a-z0-9+\/%*_.@=\-]*)"|(true|false))$/i)) || q(new sjcl.exception.invalid("json decode: this isn't json!")), g[3] ? e[g[2]] = parseInt(g[3], 10) : g[4] ? e[g[2]] = g[2].match(/^(ct|adata|salt|iv)$/) ? sjcl.codec.base64.toBits(g[4]) : unescape(g[4]) : g[5] && (e[g[2]] = "true" === g[5])
        }
        return e
    }, e: function(f, e, h) {
        f === s && (f = {});
        ...
        for (var g in e) {
            e.hasOwnProperty(g) && (h && (f[g] !== s && f[g] !== e[g]) && q(new sjcl.exception.invalid("required parameter overridden")), f[g] = e[g])
        }
        return f
    }, fa: function(f, e) {
        var h = {}, g;
